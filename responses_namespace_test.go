...
func TestResponsesNamespaceToolBridgesAndRestores(t *testing.T) {
	...
	got, err := translateToResponsesWithTools(upstream, "nvidia/z-ai~sglm-5.2", translation)
	if err != nil {
		t.Fatal(err)
	}
...
func TestResponsesNamespaceStreamingRestoresNamespace(t *testing.T) {
	...
	streamTranslateResponses(w, strings.NewReader(stream), "nvidia/z-ai~sglm-5.2", translation)
