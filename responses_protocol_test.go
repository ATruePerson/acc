...
func TestResponsesRequestPreservesUnknownFields(t *testing.T) {
	...
	if err := json.Unmarshal([]byte(`{"model":"gpt-5.6-sol","input":"hello","future_control":{"mode":"keep"}}`), &req); err != nil {
		t.Fatal(err)
	}
...
func TestResponsesReasoningSurvivesTranslation(t *testing.T) {
	...
	resp := translateToResponses(upstream, codexOpusID)
...
func TestResponsesStreamWithoutDoneIsIncomplete(t *testing.T) {
	...
	streamTranslateResponsesWithCompletion(w, strings.NewReader(stream), codexOpusID, func(response *ResponsesResponse) {
		completed = response
	})
	body := w.Body.String()
